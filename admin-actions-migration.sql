-- Admin Actions Table Migration
-- Add this to your existing Supabase database for admin elevation functionality
...
-- 7. Grant execute permission for admin check function
GRANT EXECUTE ON FUNCTION is_current_user_admin TO authenticated;

-- 8. Update existing elevation function to include admin logging
...
BEGIN
  ...
  SELECT is_current_user_admin() INTO admin_check;
...
-- 9. Grant execute permission for elevation function
GRANT EXECUTE ON FUNCTION elevate_user_subscription_admin TO authenticated;

-- 10. Create view for admin dashboard with subscription info
CREATE OR REPLACE VIEW admin_users_with_subscriptions AS
SELECT 
  p.id,
  p.first_name,
  p.last_name,
...
-- 11. Grant select permission on admin view
GRANT SELECT ON admin_users_with_subscriptions TO authenticated;

-- 12. Create policy for admin view (only admins can access)
CREATE POLICY "Only admins can view admin users view" ON admin_users_with_subscriptions
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() AND is_admin = true
